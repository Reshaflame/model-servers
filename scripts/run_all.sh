...
if [ ! -d "data/chunks" ]; then
    ...
    python -c "from utils.datachunker import DataChunker; DataChunker.split_dataset('data/auth.txt.gz', output_dir='data/chunks')"
else
...
python -c "from utils.cleanup import CleanupUtility; CleanupUtility.cleanup_chunks('data/chunks')"
